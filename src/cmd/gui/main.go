...
Default logic allows adding "_" prefix to variables that consists only from integers (no if statements, no +-* operations). It prevents from erasing your custom logic.`)
	...
	if statInput.IsDir() {
		...
		corpus.ReplaceMakroInCorpusFile(*input, *output, makrosToReplace, map[string]string{}, *alwaysConvertLocalToGlobal, *verbose, *minify) // todo support from cmd line all options
	}
